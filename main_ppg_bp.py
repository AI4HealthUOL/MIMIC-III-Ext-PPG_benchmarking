...
class Main_PPG(lp.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        elif hparams.finetune_dataset.startswith("mimic_ppg_hr") or hparams.finetune_dataset.startswith("mimic_ppg_rr")or hparams.finetune_dataset.startswith("mimic_ppg_va"):
            num_classes = 1
            self.task = "regression"
        ...
        elif(hparams.finetune_dataset == "mimic_ppg_af1"):
            num_classes = 3
            self.task = "classification"
        elif(hparams.finetune_dataset == "mimic_ppg_af2"):
            num_classes = 22
            self.task = "classification"            
    ...
    def eval_scores(self, targs,preds,classes=None):
        ...
        if self.task == "regression":
            ...
            if np.isnan(mae).any() or np.isinf(mae).any():
                print("⚠️ Warning: MAE contains NaN or Inf – returning fallback values")
                return {
                    "mae0": float("inf"), "mae1": float("inf"),
                    "gradea0": 0.0, "gradea1": 0.0,
                    "gradeb0": 0.0, "gradeb1": 0.0,
                    "gradec0": 0.0, "gradec1": 0.0,
                    "graded0": 0.0, "graded1": 0.0,
                }
            ...
            try:
                auc = roc_auc_score(targs, preds, multi_class='ovr' if preds.shape[1] > 2 else 'raise')
            except:
                auc = 0.5
    ...
    def on_valtest_epoch_eval(self, outputs_all, dataloader_idx, test=False):
        #for dataloader_idx,outputs in enumerate(outputs_all): #multiple val dataloaders
            ...
            print("✅ agg preds shape:", preds_all_agg.shape)
            print("✅ agg targs shape:", targs_all_agg.shape)
            print("✅ agg preds example:", preds_all_agg[:5])
            print("✅ agg targs example:", targs_all_agg[:5])
            print("❗ NaNs in agg preds:", np.isnan(preds_all_agg).any())
            print("❗ NaNs in agg targs:", np.isnan(targs_all_agg).any())
            print("❗ Infs in agg preds:", np.isinf(preds_all_agg).any())
            print("❗ Infs in agg targs:", np.isinf(targs_all_agg).any())
    ...
    def setup(self, stage):
        ...
        for i,target_folder in enumerate(list(self.hparams.data.split(","))):
            ...
            if self.hparams.finetune_dataset.startswith("bpbenchmark"):
                ...
                elif self.hparams.finetune_dataset.endswith("va"):
                ...
                elif self.hparams.finetune_dataset.endswith("af1"):
                    df_mapped["label"] = df_mapped.apply(lambda row: int(row["AF_1"]), axis=1)

                elif self.hparams.finetune_dataset.endswith("af2"):
                    df_mapped["label"] = df_mapped.apply(lambda row: int(row["AF_2"]), axis=1)
            ...
            if(self.hparams.disregard_splits):#ignore splits
                ...
                #col_set = self.hparams.finetune_dataset.split("_")[2]
                col_set="strat_fold"
...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(
		...
		save_last=True,
        verbose=True,
        monitor= "auc_agg_val0" if hparams.finetune_dataset=="deepbeat" or hparams.finetune_dataset=="mimic_ppg_af1" or hparams.finetune_dataset=="mimic_ppg_af2" else 'mae0_agg_val0' ,#val_loss/dataloader_idx_0
        mode='max' if hparams.finetune_dataset=="deepbeat" or hparams.finetune_dataset=="mimic_ppg_af1" or hparams.finetune_dataset=="mimic_ppg_af2" else 'min')
