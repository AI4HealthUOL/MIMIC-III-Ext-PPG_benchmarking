#deepbeat
#python main_ppg.py --data $GROUP/datasets/ppg_data/deepbeat/memmap --input-size 800 --architecture xresnet1d50 --finetune-dataset deepbeat --refresh-rate 1
#deepbeat S4
#python main_ppg.py --data $GROUP/datasets/ppg_data/deepbeat/memmap --input-size 800 --finetune-dataset deepbeat --refresh-rate 1 --architecture s4 --precision 32 --s4-n 8 --s4-h 512 --batch-size 32 --epochs 30

#mimiciiibp
#python main_ppg.py --data $GROUP/datasets/ppg_data/mimiciiibp/memmap --input-size 875 --architecture xresnet1d50 --finetune-dataset mimiciiibp --refresh-rate 1
...
#pulsedb
#python main_ppg.py --data $../mimic_PPG/memmap --input-size 1250 --architecture xresnet1d50 --finetune-dataset mimic_ppg_bp --refresh-rate 1
...
def fixed_operating_points(y_true, y_score, target_specs=[0.8, 0.9], target_sens=[0.8, 0.9]):
    """
    Computes:
    ...
    for ts in target_specs:
        idx = np.where(spec >= ts)[0]
        if len(idx) > 0:
            results[f"sens_at_spec_{int(ts*100)}"] = sens[idx[-1]]
        else:
            results[f"sens_at_spec_{int(ts*100)}"] = np.nan

    # Specificity at fixed sensitivity
    for ts in target_sens:
        idx = np.where(sens >= ts)[0]
        if len(idx) > 0:
            results[f"spec_at_sens_{int(ts*100)}"] = spec[idx[0]]
        else:
            results[f"spec_at_sens_{int(ts*100)}"] = np.nan
...
class Main_PPG(lp.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        print(hparams)
        if(hparams.finetune_dataset == "mimiciiibp" or hparams.finetune_dataset.startswith("bpbenchmark") or hparams.finetune_dataset.startswith("pulsedb") or hparams.finetune_dataset.startswith("mimic_ppg_bp")):
            num_classes = 2
            self.task = "regression"
 
        elif hparams.finetune_dataset.startswith("mimic_ppg_hr") or hparams.finetune_dataset.startswith("mimic_ppg_rr") or hparams.finetune_dataset.startswith("mimic_ppg_age") :
            num_classes = 1
            self.task = "regression"
        ...
        elif(hparams.finetune_dataset == "mimic_ppg_af1_1"):
            num_classes = 3
            self.task = "classification"
        elif(hparams.finetune_dataset == "mimic_ppg_af2_1"):
            num_classes = 12
            self.task = "classification"
        elif(hparams.finetune_dataset == "mimic_ppg_af1_2"):
            num_classes = 2 ## 3 classes - 1 class other #task 1
            self.task = "multilabel"
        elif(hparams.finetune_dataset == "mimic_ppg_af2_2"):
            num_classes = 13 ## 14 classes - 1 class other #task 2
            self.task = "multilabel"
        elif(hparams.finetune_dataset == "mimic_ppg_af3_2"):
            num_classes = 2 ## 3 classes - 1 class other #task 3
            self.task = "multilabel"             
        ...
        if(hparams.architecture=="xresnet1d50"):
            ...
            self.model = lenet1d(input_channels=hparams.input_channels, num_classes=num_classes, input_size=hparams.input_size)         
        else:
            assert(False)
    ...
    def forward(self, x, **kwargs):
            # 1. Channel Dimension Management
            if(len(x.shape)==2): # Input shape is [Batch, Length]
                x = x.unsqueeze(1) # Add an empty channel axis (now [Batch, 1, Length])
            
            # 2. Per-Window Z-Score Normalization
            if(self.hparams.normalize):
                
                # Calculate Mean (mu) for each sequence (window) along the time axis (dim=2).
                mu = x.mean(dim=2, keepdim=True)
                
                # Calculate Standard Deviation (sigma) for each sequence (window) along the time axis (dim=2).
                sigmaa = x.std(dim=2, keepdim=True)
            ...
            # 3. Pass the normalized input (x2) through the actual model
            # The line below ensures that the output is the model's prediction, NOT the input signal.
    ...
    def eval_scores(self, targs,preds,classes=None):
        ...
        if self.task == "regression":
            ...
            print("MAE debug – preds:", preds[:5])
            print("MAE debug – targs:", targs[:5])

            if np.isnan(preds).any() or np.isnan(targs).any():
                print("NaNs detected in preds or targs")

        
            # Scalar regression case (e.g., HR or RR)
            if np.ndim(mae) == 0 or (isinstance(mae, np.ndarray) and mae.shape[0] == 1):
                return {"mae0": float(mae) if np.ndim(mae) == 0 else float(mae[0])}


            # Multi-output regression (e.g., SBP and DBP)
            gradea = np.mean(maes <= 5.0, axis=0)
            gradeb = np.mean((maes > 5.0) & (maes <= 6.0), axis=0)
            gradec = np.mean((maes > 6.0) & (maes <= 7.0), axis=0)
            graded = np.mean(maes > 7.0, axis=0)

            return {
                "mae0": mae[0], "mae1": mae[1],
                "gradea0": gradea[0], "gradea1": gradea[1],
                "gradeb0": gradeb[0], "gradeb1": gradeb[1],
                "gradec0": gradec[0], "gradec1": gradec[1],
                "graded0": graded[0], "graded1": graded[1],
            }
            ...
            # Use macro-average F1 score for multiclass
            f1 = f1_score(targs, preds_hard, average="macro" if len(np.unique(targs)) > 2 else "binary")
            #try:
            ...
            try:
                # Convert targs to one-hot encoding for compatibility
                targs_onehot = np.eye(preds.shape[1])[targs]

                # Get AUCs per class using provided utility
                _, _, aucs = multiclass_roc_curve(targs_onehot, preds)

                auc_per_class = {f"auc_class_{k}": v for k, v in aucs.items() if k not in ["micro", "macro"]}
                auc_macro = aucs["macro"]
            except Exception as e:
                print("❌ Error in multiclass_roc_curve:", e)
                auc_per_class = {f"auc_class_{i}": float("nan") for i in range(preds.shape[1])}
                auc_macro = 0.5


            return {"acc":acc, "f1":f1, "auc_macro": auc_macro, **auc_per_class }
            ...
            acc = np.mean(np.all(preds_hard == targs, axis=1))
            f1 = f1_score(targs, preds_hard, average="macro")

            auc_macro = 0.5
            auc_per_class = {}
            sensitivity_per_class = {}
            specificity_per_class = {}
            fixed_metrics_per_class = {}

            try:
                auc_macro = roc_auc_score(targs, preds, average="macro")
                ...
                for i in range(preds.shape[1]):
                    ...
                    # AUC
                    auc_per_class[f"auc_class_{i}"] = roc_auc_score(y_true, y_score)

                    # --- Threshold 0.5 metrics ---
                    tp = np.sum((preds_hard[:, i] == 1) & (y_true == 1))
                    tn = np.sum((preds_hard[:, i] == 0) & (y_true == 0))
                    fp = np.sum((preds_hard[:, i] == 1) & (y_true == 0))
                    fn = np.sum((preds_hard[:, i] == 0) & (y_true == 1))

                    sensitivity = tp / (tp + fn + 1e-8)
                    specificity = tn / (tn + fp + 1e-8)

                    sensitivity_per_class[f"sens_class_{i}"] = sensitivity
                    specificity_per_class[f"spec_class_{i}"] = specificity

                    # ---Fixed Operating Points ---
                    fixed_ops = fixed_operating_points(y_true, y_score)

                    for k, v in fixed_ops.items():
                        fixed_metrics_per_class[f"{k}_class_{i}"] = v

            except Exception as e:
                print("❌ Error in multilabel metrics:", e)
    ...
    def on_valtest_epoch_eval(self, outputs_all, dataloader_idx, test=False):
        #for dataloader_idx,outputs in enumerate(outputs_all): #multiple val dataloaders
            ...
            # Threshold after aggregation for multilabel targets
            if self.task == "multilabel":
                targs_all_agg = (targs_all_agg > 0.5).astype(np.int32)
    ...
    def setup(self, stage):
        ...
        for i,target_folder in enumerate(list(self.hparams.data.split(","))):
            ...
            elif(self.hparams.finetune_dataset.startswith("mimic_ppg")):
                if(self.hparams.finetune_dataset.endswith("bp")):
                    df_mapped=df_mapped[df_mapped.bp==1].copy()
                elif(self.hparams.finetune_dataset.endswith("hr")):
                    df_mapped=df_mapped[df_mapped.hr==1].copy()
                elif(self.hparams.finetune_dataset.endswith("rr")):
                    df_mapped=df_mapped[df_mapped.rr==1].copy()
                elif(self.hparams.finetune_dataset.endswith("af1_2")): # excluding LBBB and RBBB
                    df_mapped=df_mapped[df_mapped.AF_1_10>-1].copy()    
                elif(self.hparams.finetune_dataset.endswith("af2_2")): # excluding LBBB and RBBB
                    df_mapped=df_mapped[df_mapped.AF_2_10>-1].copy()
                elif(self.hparams.finetune_dataset.endswith("af3_2")): # excluding LBBB and RBBB
                    df_mapped=df_mapped[df_mapped.AF_3_10>-1].copy()                     
            ...
            #if(self.hparams.finetune_dataset.startswith("bpbenchmark") or self.hparams.finetune_dataset.startswith("pulsedb") or self.hparams.finetune_dataset.startswith("mimic_ppg")):#WORKAROUND
            ...
            if self.hparams.finetune_dataset.startswith("bpbenchmark"):
                   df_mapped["sp"] = df_mapped["sp"].astype(np.float32)
                   df_mapped["dp"] = df_mapped["dp"].astype(np.float32)
                   df_mapped["label"]=df_mapped.apply(lambda row: np.array([row["sp"],row["dp"]],dtype=np.float32), axis=1)
            elif self.hparams.finetune_dataset.startswith("mimic_ppg"):
                if self.hparams.finetune_dataset.endswith("bp"):
                
                    df_mapped["sp"] = df_mapped["median_30s_sbp"].astype(np.float32)
                    df_mapped["dp"] = df_mapped["median_30s_dbp"].astype(np.float32)
                    df_mapped["label"] = df_mapped.apply(lambda row: np.array([row["sp"], row["dp"]], dtype=np.float32), axis=1)
                elif self.hparams.finetune_dataset.endswith("hr"):
                    
                    df_mapped["hr"] = df_mapped["median_30s_hr"].astype(np.float32)
                    df_mapped["label"]=df_mapped.apply(lambda row: np.array(row["hr"],dtype=np.float32), axis=1)
                    
                elif self.hparams.finetune_dataset.endswith("rr"):
                    df_mapped["rr"] = df_mapped["median_30s_rr"].astype(np.float32)
                    df_mapped["label"]=df_mapped.apply(lambda row: np.array(row["rr"],dtype=np.float32), axis=1)
                elif self.hparams.finetune_dataset.endswith("age"):
                    df_mapped["age"] = df_mapped["age"].astype(np.float32)
                    df_mapped["label"]=df_mapped.apply(lambda row: np.array(row["age"],dtype=np.float32), axis=1)    
                
                    
                    
                elif self.hparams.finetune_dataset.endswith("af1_2"):

                    
                    ## using multigot encoder to testing teh senario where we have 11 normal arythmia clases and the rest wil be the class 0..
                    ## so we will not try to find the algorithm insdie the class 0..and classs 0 will be just the rest
                    
                    df_mapped["label"] = df_mapped["AF_1_10"].apply(lambda x: multihot_encode(x, num_classes_except_0=2))                    
  
                    
                elif self.hparams.finetune_dataset.endswith("af2_2"):

                   
                    ## using multigot encoder to testing teh senario where we have 11 normal arythmia clases + the rest wil be the class 0..
                    ## so we will not try to find the algorithm insdie the class 0..and classs 0 will be just the rest          
                    df_mapped["label"] = df_mapped["AF_2_10"].apply(lambda x: multihot_encode(x, num_classes_except_0=13)) ## infact the number of classes -1
                    
                    ## in the final results just please 
                    
                elif self.hparams.finetune_dataset.endswith("af3_2"):
                
                    ## using multigot encoder to testing teh senario where we have 11 normal arythmia clases and the rest wil be the class 0..
                    ## so we will not try to find the algorithm insdie the class 0..and classs 0 will be just the rest
                    
                    df_mapped["label"] = df_mapped["AF_3_10"].apply(lambda x: multihot_encode(x, num_classes_except_0=2))   
            ...
            else:  # fallback for pulsedb or others
                   df_mapped["sp"] = df_mapped["sbp_avg"].astype(np.float32)
                   df_mapped["dp"] = df_mapped["dbp_avg"].astype(np.float32)
                   df_mapped["label"]=df_mapped.apply(lambda row: np.array([row["sp"],row["dp"]],dtype=np.float32), axis=1)
            ...
            if(self.hparams.disregard_splits):#ignore splits
                ...
                col_set="strat_fold"
                df_train = df_mapped[(df_mapped[col_set]<=6)]
                df_val = df_mapped[df_mapped[col_set].isin([7])]
                df_tests = [df_mapped[df_mapped[col_set].isin([8,9])]]
            ...
            train_datasets.append(TimeseriesDatasetCrops(df_train,self.hparams.input_size,data_folder=target_folder,chunk_length=chunk_length_train,min_chunk_length=self.hparams.input_size, stride=stride_train,transforms=tfms,col_lbl ="label",col_data="signal_file_name" ,memmap_filename=target_folder/("memmap.npy")))
            val_datasets.append(TimeseriesDatasetCrops(df_val,self.hparams.input_size,data_folder=target_folder,chunk_length=chunk_length_valtest,min_chunk_length=self.hparams.input_size, stride=stride_valtest,transforms=tfms,col_lbl ="label",col_data="signal_file_name",memmap_filename=target_folder/("memmap.npy")))
            for df_test in df_tests:
                test_datasets.append(TimeseriesDatasetCrops(df_test,self.hparams.input_size,data_folder=target_folder,chunk_length=chunk_length_valtest,min_chunk_length=self.hparams.input_size, stride=stride_valtest,transforms=tfms,col_lbl ="label",col_data="signal_file_name", memmap_filename=target_folder/("memmap.npy")))
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.hparams.batch_size, num_workers=32, shuffle=True, drop_last = True, pin_memory=True, persistent_workers=True)
        
    def val_dataloader(self):
        return [DataLoader(ds, batch_size=self.hparams.batch_size, num_workers=32, pin_memory=True, persistent_workers=True) for ds in self.val_datasets]
    
    def test_dataloader(self):
        return [DataLoader(ds, batch_size=self.hparams.batch_size, num_workers=32, pin_memory=True, persistent_workers=True) for ds in self.test_datasets]
    ...
    def _step(self,data_batch, batch_idx, train, test=False, dataloader_idx=0):
        ...
        #if self.task == "multilabel":
        #    print(f"[DEBUG] preds_all shape: {preds_all.shape}")
        #    print(f"[DEBUG] data_batch[1] shape: {data_batch[1].shape}")
        #    print(f"[DEBUG] data_batch[1][0]: {data_batch[1][0]}")
        #    print(f"[DEBUG] type: {type(data_batch[1][0])}")
        #    print(f"[DEBUG] unique values in batch: {torch.unique(data_batch[1])}")

        
        
        
        #print("Preds shape:", preds_all.shape)
        #print("Labels shape:", data_batch[1].shape)
        
        # DEBUG: Check for NaNs in inputs and predictions
        if torch.isnan(data_batch[0]).any():
            print("❌ NaNs in input data_batch[0]!")

        if torch.isnan(preds_all).any():
            print("❌ NaNs in model output (preds_all)!")
            print("Sample input:", data_batch[0][0])
            print("Model output:", preds_all[0])
            raise ValueError("NaNs in predictions – exiting early")
        if torch.isnan(data_batch[1]).any():
            print("❌ NaNs in target data_batch[1]!")

        
        if self.task == "regression" and preds_all.shape != data_batch[1].shape:
            data_batch = (data_batch[0], data_batch[1].unsqueeze(1))
            
        if preds_all.shape != data_batch[1].shape and self.task == "multilabel":
            raise ValueError(f"Shape mismatch for multilabel task. Preds: {preds_all.shape}, Labels: {data_batch[1].shape}")
        ...
        # For mimiciiibp, flatten BP arrays for regression
        if self.hparams.finetune_dataset == "mimiciiibp":
            loss = self.criterion(preds_all.view(-1), data_batch[1].view(-1))

        # For multilabel: cast targets to float (multi-hot format)
        elif self.task == "multilabel":
            loss = self.criterion(preds_all, data_batch[1].float())
    ...
    def load_state_dict_OLD(self, state_dict):
...
def multihot_encode(label_index, num_classes_except_0):
    vec = np.zeros(num_classes_except_0, dtype=np.float32)
    if label_index > 0:
        vec[label_index - 1] = 1.0
    return vec
...
if __name__ == '__main__':
    ...
    hparams.executable = "mimic_ppg_v1_1"
    ...
    logger = [TensorBoardLogger(
        ...
        name="",                       # keeps folder flat
        version=hparams.metadata       # 👈 THIS is the key line
    )]
    ...
    if(MLFLOW_AVAILABLE):
        try:
            ...
            print(f"[WARNING] MLflow logging is skipped due to error: {e}")
    ...
    checkpoint_callback = ModelCheckpoint(
		...
		save_last=True,
        verbose=True,
        monitor= "auc_macro_agg_val0" if hparams.finetune_dataset=="deepbeat" or hparams.finetune_dataset=="mimic_ppg_af1_1" or hparams.finetune_dataset=="mimic_ppg_af2_1" or hparams.finetune_dataset=="mimic_ppg_af1_2" or hparams.finetune_dataset=="mimic_ppg_af2_2" or hparams.finetune_dataset=="mimic_ppg_af3_2" else 'mae0_agg_val0' ,#val_loss/dataloader_idx_0
        mode='max' if hparams.finetune_dataset=="deepbeat" or hparams.finetune_dataset=="mimic_ppg_af1_1" or hparams.finetune_dataset=="mimic_ppg_af2_1" or hparams.finetune_dataset=="mimic_ppg_af1_2" or hparams.finetune_dataset=="mimic_ppg_af2_2" or hparams.finetune_dataset=="mimic_ppg_af3_2" else 'min')
